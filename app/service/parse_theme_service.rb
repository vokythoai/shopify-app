  ...
  class << self
    ...
    def add_discount_cart html_content, promotions, shop
        ...
        alert_spend_qty.each_with_index do |detail, index_|
          ...
          @alert_spend_amount_html += '<script type="text/javascript">' +
                                      'script = document.createElement("script");
                                        ...
                                        'var html = "' + "<div id='miskre-notification-bar' style='display: block;'>Spend " + "{{ #{detail[0].to_i*100} | money }}" + " to get #{detail[1].to_i}% off<div id='miskre-close-notification'>X</div></div>" + '";' +
                                        '$("main").prepend(html);' +
                                        '};' +
                                      '</script>'
        end
      ...
      else_spend_amount = "{% elsif true %}
                          ...
                          {% endif %}"
      total_qty = '<span class="cart__subtotal"><span class="wh-original-cart-total">{{ total | money }}</span>' + (@spend_amount_html + else_spend_amount) + '</span><div class="additional-notes">YOU SAVE {{ original_total | minus: total | money}}</div></span>' + @alert_spend_amount_html
